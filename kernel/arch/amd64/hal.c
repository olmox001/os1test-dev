...
void timer_delay_ms(uint64_t ms) {
  ...
  if (ticks_per_ms > 0) {
    uint64_t start = lapic_read(LAPIC_TCC);
    uint64_t ticks = ms * ticks_per_ms;
    /* LAPIC timer counts down */
    while ((start - lapic_read(LAPIC_TCC)) < ticks) {
      hal_cpu_yield();
    }
  } else {
    /* Fallback if APIC timer not yet calibrated */
    for (uint64_t i = 0; i < ms * 1000000; i++) {
      hal_cpu_yield();
    }
  }
}
