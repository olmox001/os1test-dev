...
struct window {
  ...
  /* Compositor flags */
  int has_alpha; /* Se 1, contains transparency and does not occlude lower layers */
};
  ...
  for (int i = 0; i < MAX_WINDOWS; i++) {
    if (windows[i].id == window_id) {
      if (windows[i].pid == keyboard_focus_pid) {
        /* Focused window is being destroyed, reset focus to Shell */
...
static void compositor_render_internal(void) {
  ...
  for (int i = 0; i < current_count; i++) {
    if (sorted[i]->top_most) {
      ...
      current_count--;
      /* Decrement i to process the shifted window */
      i--;
    }
  }
  ...
  for (int i = count - 1; i >= 0 && i < MAX_WINDOWS; i--) {
    ...
    /* Add to Occluded (Only if the window has no transparency) */
    if (!win->has_alpha) {
  ...
  for (int i = 0; i < count && i < MAX_WINDOWS; i++) {
    ...
    /* Draw Title Text - Naive Unclipped */
    if (!win->top_most) {
      int title_len = 0;
      ...
      int start_y = decor_y + (20 - char_h) / 2;
  ...
  /* Flush damage bounding box instead of the full framebuffer */
