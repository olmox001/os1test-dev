...
struct process *process_create(const char *name, uint8_t priority,
  ...
  strncpy(proc->root, "/", sizeof(proc->root));
...
int sys_ipc_send(int target_pid, void *msg_ptr) {
  ...
  int ret = kernel_ipc_send(target_pid, &k_msg);
  
  if (ret == 0) {
      /* Optional: yield if we want synchronous behavior or the target is high priority */
...
int sys_ipc_recv(int src_pid, void *msg_ptr) {
  ...
  if (node) {
    if (vmm_copy_to_user(msg_ptr, &node->msg, sizeof(struct ipc_message)) != 0) {
      /* Return node to queue or just drop? Drop for now to avoid complexity */
      kfree(node);
      return -EFAULT;
    }
    kfree(node);
  ...
  /* IMPORTANT: Record where the user wanted the message to be copied 
   * so we can resume later if we use synchronous recv. 
   * But for now, we just retry the syscall. */
  
  spin_unlock_irqrestore(&cpu->sched_lock, flags);

  /* Retry the syscall instruction on wake-up so it calls sys_ipc_recv again */
