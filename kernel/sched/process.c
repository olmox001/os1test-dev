...
int process_terminate(int pid) {
  ...
  if (proc->wait_queue_ptr) {
    ...
    /* CRITICAL: Acquire target CPU's sched_lock before removing from its
     * runqueue */
    int t_id = (proc->on_cpu >= 0) ? proc->on_cpu : 0;
    struct cpu_info *t_cpu = &cpu_data[t_id];

    /* We already hold the global sched_lock, but we must also hold the per-CPU
     * lock to prevent races with the scheduler on that CPU. */
...
void start_user_process(struct process *proc) {
  ...
  /* Set current_process via per-CPU structure (CRITICAL FIX) */
