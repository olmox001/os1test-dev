...
void vfs_trace(const char *op, const char *path, size_t bytes);

/* Trace helper for VFS */
void vfs_trace(const char *op, const char *path, uint64_t bytes) {
  if (bytes > 0) {
    pr_debug(" VFS: %s %s (%lu bytes)\n", op, path, bytes);
  } else {
    pr_debug(" VFS: %s %s\n", op, path);
  }
}
