...
/*
 * Look up a vnode by (mount, inode). Bumps usecount while holding the hash
 * lock so the vnode cannot be reclaimed before the caller has a reference.
 * Returns 0 and sets *vpp on hit; returns -ENOENT on miss.
 */
