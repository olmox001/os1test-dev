...
#include <core/multiboot2.h>
#include <hal/drivers/timer.h>

void arch_platform_early_init(void) {
  uart_init();
  pr_info("AMD64 Platform Initialization (Magic: 0x%x, Info: 0x%x)\n",
  ...
  if (mb_magic == MB1_MAGIC) {
    /* Multiboot v1 */
    struct mb1_info *mb1 = (struct mb1_info *)(uintptr_t)mb_info_ptr;
        ...
        for (uint32_t i = 0; i < num_entries && arch_region_count < 32; i++) {
          struct mb2_mmap_entry *entry =
              (struct mb2_mmap_entry *)((uint8_t *)mmap->entries +
                                        i * entry_size);
          pr_info("Mmap Entry %u: Base 0x%lx, Size 0x%lx, Type %u\n", i,
    ...
    /* PVH boot */
    struct hvm_start_info *pvh = (struct hvm_start_info *)(uintptr_t)mb_info_ptr;
    ...
    pr_warn("AMD64: [IDTF] Unknown boot protocol (Magic: 0x%x). Using safe "
            "1GB default.\n",
            mb_magic);
